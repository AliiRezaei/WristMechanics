...
```matlab
>> run motion/kinematics/WristKinematics.m % kinematic eqs
>> run motion/dynamics/WristDynamics.m     % dynamic   eqs
```

These scripts compute the robot's kinematic and dynamic equations and export the resulting matrices in multiple formats supported by **ManiDyn**. For additional information, see the `ReturnFormat` documentation in ManiDyn.
...
- **Control**
  ...
  ```matlab
  >> sim examples/control/InverseDynamics/WristControlID.slx
  ```
  - Model-based PD + Gravity compensator.
  ```matlab
  >> sim examples/control/PDplusGravity/WristControlPDG.slx
  ```
...
A quick README for the control schemes can be found in [Kinova-Gen3-Control](https://github.com/BanaanKiamanesh/Kinova-Gen3-Control). All example models were developed using **MATLAB R2025b**.
